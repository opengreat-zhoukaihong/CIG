...
<%!
    String memberID,countryID,provinceID,cityID,shipName,address,zipCode,contName;
...
<%
          tempSqlStr = "insert into shipment (shipID,memberID,countryID,provinceID, " +
            " cityID,zipCode,tel,fax,email,cr_date,md_date) " +
            " values (seq_shipmentID.NextVal,'" + memberID + "','" + countryID +
            "','" + provinceID +"','" + cityID + "','" + zipCode + "','" + telephone +
            "','" + fax + "','" + contEmail + "',sysdate,sysdate )";

          shipManager.setSqlStr(tempSqlStr);

          tempSqlStr = "insert into shipment_L (shipID,lang_code,contName_F,shipName,address1)" +
            " values (seq_shipmentID.CurrVal,'GB','" + contName + "','" + shipName + "','" +
            address +"')";

          shipManager.setSqlStr2(tempSqlStr);

          shipManager.modifyShip();
          errorCode = shipManager.getErrorCode();
