...
## Output path
$path = "/usr/local/etc/w3perl/htdocs/demo/";

## Path where the package have been installed
$pathinit = "/usr/local/etc/w3perl/htdocs/demo/";

## Path where your HTML documents are stored
$pathserver = "/usr/local/etc/w3perl/htdocs/";

## Absolute URL where your HTML documents are located (usually /)
$linkpathserver = "/";

## Cgi-bin path
$cgipath = "/usr/local/etc/w3perl/cgi-bin/";

## Path where log files are located on your server
$fileroot = "/usr/local/etc/w3perl/logs/futurequest/";
...
@selection = ('/download/index.html','/astro/seti/setiathome/index.html');

## Subdirectories with graphical stats output
## (URL from your Document root)
@selecrepert = ('/acorn/','/astro/','/demo/','/domisse/','/download/','/fun/','/fly/','/news/','/softs/','/www/');

## Excluding subdirectories from your report
## (URL from your Document root)
@excluderepert = ('/old/','/private/','/carpentier/','/nabti/','/propals/','/stehelin/');
...
$optdirsize = 1;    # directories traffic graphs [html pages ( 1 : total, 2 : external, 3 : local), all files ( 4 : total, 5 : external, 6 : local)]
$nbdays = 50;       # number of days to display for graphs (0 => one year)

@extension = ('html','htm','shtml');    # html pages extensions to parse
@extensionimage = ('gif','jpeg','jpg'); # images extensions to parse

$reverse_dns = 1; # Reverse IP to name (slow down computing by a large amount)
...
$localdomainename = "w3perl.com";
$localdomaine = "w3perl.com";

$emailadr = "domisse\@w3perl.com"; # your email here (default is webmaster's email)
...
@homepages = ('index-fr','index');
@lang = ('fr','uk');

# Your own link in the top frame
$topframelinks = "<A HREF='http://$localserver$tri' TARGET='_top'>W3Perl</A>";
...
# cron-url.pl
$url_frame = 1;         # HTML pages without TITLE tag
...
$url_cdrom = 1;         # Show HTML pages without index.html link
