<%@ page errorPage="ErrorPage.jsp" import="java.util.*, com.forbrand.show.TypeBean"%> 
...
<table width=758 border=0 cellpadding=0 cellspacing=0>
  ...
  <tr> <!-- #BeginEditable "replace" --> 
    <td colspan=3 rowspan=3 background="images/background1_05.gif" class="orange-title1"> 
      <blockquote> WELCOME TO OUR SHOWROOM!<br>
  ...
  <tr> 
    <td> 
      ...
      <table width="666" border="0" cellspacing="8" align="left">
        <tr> 
          <td rowspan="<%= typeList.size() %>" valign="top"> 
            <div align="right"></div>
            <div align="right"></div>
          ...
          %> 
          <td width="466" valign="top" > <span class="orange-title"><%= showType.getTypeName() %><br>
            </span> <span class="legal-notice"> <%
                Iterator iter = categories.keySet().iterator();
                while (iter.hasNext()) {
                  String cateName = (String) iter.next();
                  String cateID = (String) categories.get(cateName);
              %> <a href="showinside.jsp?act=0&cateID=<%= cateID%>&langCode=<%= showType.getLangCode() %>"><%= cateName %></a><img src="/images/5x1.gif" width="5" height="1"> 
            <%
                }//while iter
              %> </span> </td>
        </tr>
