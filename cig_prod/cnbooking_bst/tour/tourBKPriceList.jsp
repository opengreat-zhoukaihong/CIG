...
<p><span class="txt">共有<%=lineCount%>条旅游线路,共<%=totalPage%>页&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;第<%=pageno%>页 
                <%if ((pageno == 1)&&(totalPage>1)){%><a href="tourBKPriceList.jsp?pageNo=2&ope_ID=<%=ope_ID%><%=str%>">下一页</a>
                <%}else if ((pageno>1)&&(pageno<totalPage)){%><a href="tourBKPriceList.jsp?pageNo=<%=pageno-1%>&ope_ID=<%=ope_ID%><%=str%>">上一页</a><a href="tourBKPriceList.jsp?pageNo=<%=pageno+1%>&ope_ID=<%=ope_ID%><%=str%>">|下一页</a>
              <%}else if ((pageno>1)&&(pageno==totalPage)){%>
              <a href="tourBKPriceList.jsp?pageNo=<%=pageno-1%>&ope_ID=<%=ope_ID%><%=str%>">上一页</a><%  }%>|<a href="tourBKPriceList.jsp?pageNo=1&ope_ID=<%=ope_ID%><%=str%>">回到首页</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="tourBKPriceList.jsp?pageNo=<%=pageno%>&ope_ID=<%=ope_ID%><%=str%>">刷新浏览</a></span></p>
...
<form name="BKList">
  <tr> 
    ...
    <td width="48%" height="20" class="txt"><font color="#666666">旅游主题</font></td>
    <td width="8%" height="20" class="txt"><font color="#666666">城市名</font></td>
    <td width="20%" height="20" class="txt"><font color="#666666">咨询电话</font></td>
    <td width="8%" height="20" class="txt"><font color="#666666">操作员</font></td>
    <td width="8%" height="20" class="txt"><font color="#666666">修改</font></td>
  </tr>
  <%int maxNum = TourBKLineList.restriction;            
    ...
    <a href="tourBKPriceView.jsp?line_ID=<%=lineLists[i][0]%>&ope_ID=<%=URLEncoder.encode(ope_ID)%>" onClick="return zy_callpage(this.href);"> 
    <%=lineLists[i][0]%></a></td>
    <td width="48%" height="20" class="txt"><%=lineLists[i][1]%> </td>
    <td width="8%" height="20" class="txt"><%=lineLists[i][4]%> </td>
    <td width="20%" height="20" class="txt"><%=lineLists[i][5]%> </td>
    <td width="8%" height="20" class="txt"><%=lineLists[i][2]%> </td>
    <td width="8%" height="20" class="txt"><a href="tourBKPriceView.jsp?line_ID=<%=lineLists[i][0]%>&ope_ID=<%=URLEncoder.encode(ope_ID)%>" onClick="return zy_callpage(this.href);">修改 </a> </td>
    </tr>
    <%
    }
    }
...
<%}else if ((pageno>1)&&(pageno<totalPage)){%><a href="tourBKPriceList.jsp?pageNo=<%=pageno-1%>&ope_ID=<%=ope_ID%><%=str%>">上一页</a><a href="tourBKPriceList.jsp?pageNo=<%=pageno+1%>&ope_ID=<%=ope_ID%><%=str%>">|下一页</a>
<%}else if ((pageno>1)&&(pageno==totalPage)){%>
<a href="tourBKPriceList.jsp?pageNo=<%=pageno-1%>&ope_ID=<%=ope_ID%><%=str%>">上一页</a><%  }%>|<a href="tourBKPriceList.jsp?pageNo=1&ope_ID=<%=ope_ID%><%=str%>">回到首页</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="tourBKPriceList.jsp?pageNo=<%=pageno%>&ope_ID=<%=ope_ID%><%=str%>">刷新浏览</a>
 </span></p>
...
<input type=hidden name="currPage"  value="tourBKPriceList.jsp">
