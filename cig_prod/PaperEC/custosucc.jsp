...
<jsp:useBean id="sendM2" scope="page" class="mypaperec.SendCustom" />
...
<jsp:setProperty name="sendM2" property="user_id" value="<%=user_id%>" />
<jsp:setProperty name="sendM2" property="posting_id" value="<%=posting_id%>" /> 
        ...
        <tr bordercolor="#FFFFFF" valign="top"> 
          <td height="172"> 
            <table width="300" border="0" cellspacing="0" cellpadding="0" align="left">
              <% if(sendM2.isMaster()){ 
        int length=Integer.valueOf(request.getParameter("length")).intValue();   
        Vector ve=new Vector();  String str=""; String id="";
        for(int i=0;i<length;i++){
           str="check"+i;       
           id=request.getParameter(str);
           if(id!=null)ve.addElement(id);
                 }    
        str=sendM2.getSendMess(ve);      
        if(str.equals("0")){
           str="数据库操作有误，请稍候再试！";
        }else{
           str="您已成功地向您关心的客户发送了信息！";
        }            %> 
              <tr align="center" bgcolor="#E6EDFB"> 
                <td class="font9" height="185"><%=str%></td>
              </tr>
              <tr bgcolor="#4078E0" align="center"> 
                <td height="25"> 
                  <input type="button" name="Submit2" value="返 回" onclick="winback()">
                </td>
              </tr>
              <%  }else{   %> 
              <tr align="center" bgcolor="#E6EDFB"> 
                <td class="font9" height="185">很抱歉！您并没有发布此定单。</td>
              </tr>
              <tr bgcolor="#4078E0" align="center"> 
                <td height="25"> 
                  <input type="button" name="Submit2" value="返 回" onclick="winback();">
