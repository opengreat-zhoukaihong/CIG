<html>
<head>
<!-- #BeginEditable "doctitle" -->
<title>い翠澳酒┌訂┬專家</title>
<!-- #EndEditable --> 
<meta http-equiv="Content-Type" content="text/html; charset=big5">
<style type="text/css">
<!--
 input {font-family: "燦　砰"; font-size: 12px ;color:#666666}
 select {font-family: "燦　砰"; color:#666666}
 A:link {text-decoration: none; color: #715922; font-family: 燦　砰}
 A:visited {text-decoration: none; color: #715922; font-family: 燦　砰}
 A:active {text-decoration: none; font-family: 燦　砰}
 A:hover {text-decoration: underline; color:#CAA54D}
 td {font-family: "燦　砰", "Times New Roman";font-size: 12px;}
 .email { font-size: 7.5pt}
 .title { font-size: 14pt; font-weight: bold}
 .txt { font-size: 9pt; line-height: 12pt}
 .bigtxt { font-size: 11pt; line-height: 15pt}
 .copyright { font-size: 8pt; color: #000000}
.star { font-size: 9px; color: #FF8000}
.pt9 {  font-family: "燦　砰"; font-size: 12px}
-->
</style>

</head>
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="5">
<script language="JavaScript" src="/javascript/big/title.js">
</script>
<table width="665" border="0" cellspacing="0" cellpadding="0" align="center" height="34">
  <tr>
    <td width="664" height="34">
      <table width="664" border="0" cellspacing="0" cellpadding="0" height="34">
        <tr>
          <td width="30" background="/images/top_back.gif" height="34" valign="bottom"><a href="/cnbooking/big/index.jsp"><img src="/images/home.gif" width="14" height="14" align="right" border="0"></a></td>
          <td width="200" background="/images/top_back.gif" height="34" valign="bottom"><!-- #BeginEditable "top" --><font color="#666666">==&gt;&gt;提醒注冊</font><!-- #EndEditable --></td>
          <td width="415" background="/images/top_back.gif" height="34">&nbsp;</td>
          <td width="19" height="34"><img src="/images/top_right.gif" width="19" height="34"></td>
        </tr>
      </table>
    </td>
  </tr>
</table>
<table width="665" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr> 
    <td colspan="3" valign="top"><!-- #BeginEditable "main" --> <br>
      <table width=400 border=0 cellpadding=0 cellspacing=0 bgcolor=#FFBC04 align="center">
        <tr align="center"> 
          <td height="3" bgcolor="#CFC8CF"><img src="/images/space.gif" width="1" height="3"></td>
        </tr>
        <tr align="center"> 
          <td><b><span class="bigtxt"><font color="#666666">對不起，您還沒有登錄，或者已經超時! </font></span></b></td>
        </tr>
        <tr align="center"> 
          <td bgcolor="#CFC8CF" height="3"><img src="/images/space.gif" width="1" height="3"></td>
        </tr>
      </table>
      <br>
      <table width="400" border="0" align="center">
        <tr> 
<script language="JavaScript">
<!--
function PostGetForm()
{ 
 if(document.LoginForm.username.value.length ==0 ){
	  alert("請輸入姓名 !\n");
          document.LoginForm.username.focus();
          return;
 }
 document.LoginForm.passwd.value="";
	document.LoginForm.submit();
}
function PostForm()
{ 
 if(document.LoginForm.username.value.length ==0 ){
	  alert("請輸入姓名 !\n");
          document.LoginForm.username.focus();
          return;
 }
 if(document.LoginForm.passwd.value.length ==0 ){
	  alert("請輸入密碼 !\n");
          document.LoginForm.passwd.focus();
          return;
 }
	document.LoginForm.submit();
}
//-->
</script>
          <form name="LoginForm" action="/cnbooking/big/CnBookingLogin.jsp?answerInput=" method="POST">
            <td align="center"> 
              <table border="0" width="134" cellspacing="0" cellpadding="0" bgcolor="#CFC8CF">
                <tr> 
                  <td height="13" align="center"><img src="/images/login_top.gif" alt="entry-1.gif (177 bytes)" width="134" height="13"></td>
                </tr>
                <tr align="center" bgcolor="#FFDA9D"> 
                  <td> 會員： 
                    <input type="text" name="username" size="11" style="font-family: 宋体; font-size: 9pt">
                  </td>
                </tr>
                <tr align="center" bgcolor="#FFDA9D"> 
                  <td> 密碼： 
                    <input type="password" name="passwd" size="11" style="font-family: 宋体; font-size: 9pt">
                  </td>
                </tr>
                <tr align="center" bgcolor="#FFDA9D"> 
                  <td height="3"><img src="/images/space.gif" width="1" height="10"> 
                  </td>
                </tr>
                <tr bgcolor="#FFDA9D"> 
                  <td height="8" align="center"><a href="javascript:PostForm()"><font color="#000000">登錄</font></a>&nbsp;<a href="javascript:PostGetForm()"><font color="#000000">取回密碼</font></a></td>
                </tr>
                <tr> 
                  <td height="9" valign="top"><img src="/images/login_down.gif" alt="entry-2.gif (158 bytes)" width="134" height="9"></td>
                </tr>
              </table>
            </td>
          </form>
          <td align="center"> 
            <table width="210" border="1" cellspacing="0" cellpadding="8" height="130" bordercolor="#FFBC04">
              <tr align="center" valign="top"> 
                <td>如果您還沒有注冊成為我們的會員,<br>
                  請立即進入 <a href="/cnbooking/big/free_register.jsp"><b><font color="#000000">免費注冊</font></b></a></td>
              </tr>
            </table>
          </td>
        </tr>
      </table>
      <!-- #EndEditable --></td>
  </tr>
  <tr>
    <td colspan="3" height="20">&nbsp;</td>
  </tr>
  <%@ include file="foot_tel.jsp"%>  
  <tr> 
    <td colspan="3">&nbsp;</td>
  </tr>
  <script language="JavaScript" src="/javascript/big/foot.js">
</script>
</table>
</body>
<!-- #EndTemplate --></html>
