...
if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
  ...
  function processOrder(tempOrderID)
  {
    ...
    document.fmProcessOrder.action = "processOrder_eoa.jsp";
  ...
  {
    document.fmProcessOrder.orderID.value = tempOrderID;
    document.fmProcessOrder.action = "orderContent_eoa.jsp";
    document.fmProcessOrder.submit();
  }
...
<%!
  ...
  String status,sellerName,orderDateBgn,orderDateEnd,searchOrderID,buyerName;
  String statusName;
...
<%
  ...
  // status = 100, indicates delayed new orders

  sellerName = request.getParameter("supplierName");
  buyerName = request.getParameter("dealerName");
  ...
  orderSearch.setSellerName(sellerName);
...
<FORM NAME="browseList" action="searchOrdersRes_eoa.jsp" METHOD="post" >
  <INPUT TYPE="HIDDEN" NAME="pageNo" >
  <input type="hidden" name="searchOrderID" value="<%=searchOrderID%>">
  <input type="hidden" name="supplierName" value="<%=sellerName%>">
  <input type="hidden" name="dealerName" value="<%=buyerName%>">
...
<form name="fmSubOrder" method="post" action="orderDetails_eoa.jsp">
  <input type="hidden" name="orderID">
  <input type="hidden" name="status" value="<%=status%>">
</form>
...
<%                 if(status.equals("100"))  {
%>                    <td><a href="javascript:processOrder('<%=myOrder.getOrderID()%>')">提醒供应商</a></td>
                <% }   %>
                      ...
                      <td><a href="viewOrdersHis.jsp?orderID=<%=myOrder.getOrderID()%>">历史记录</a></td>
                    </tr>
<%          }
            else
            {
