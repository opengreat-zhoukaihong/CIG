...
<%!
  String lang_code="GB";
  String tempSqlStr;
  String deliveryID,supplierID,countryID,provinceID,cityID;
  String zipCode,telephone,fax,contEmail,contTitle,note;
  String deliveryName,address,contName;
  int errorCode;
%>

<jsp:useBean id="modifyDelivery" scope="page" class="com.cig.chinaeoa.member.DeliveryManager" />

<%
        deliveryID = request.getParameter("deliveryID");
        supplierID = request.getParameter("supplierID");

        countryID = request.getParameter("country").trim();
        provinceID = request.getParameter("province").trim();
        cityID = request.getParameter("city").trim();
        deliveryName = request.getParameter("deliveryName").trim();
        ...
        tempSqlStr = "update delivery set countryID = '" + countryID +
          "',provinceID = '" + provinceID + "',cityID = '" + cityID +
          "',zipCode = '" + zipCode + "',tel = '" + telephone +
          "',fax = '" + fax + "',email = '" + contEmail +
          "',md_date = sysdate where deliveryID = '" + deliveryID + "'";

        modifyDelivery.setSqlStr(tempSqlStr);

        tempSqlStr = "update delivery_L set name = '" + deliveryName +
          "',contact_F = '" + contName + "',address1 = '" + address +
          "',contTitle = '" + contTitle + "',note = '" + note +
          "' where deliveryID = '" + deliveryID + "' and lang_code = '" + lang_code + "'";

        modifyDelivery.setSqlStr2(tempSqlStr);

        modifyDelivery.modifyDelivery();
        errorCode = modifyDelivery.getErrorCode();
        if(errorCode==0)
        {
%>
            <p>操作成功!</p>
            <p><a href="deliveryList.jsp?action=modify">返回</a></p>
