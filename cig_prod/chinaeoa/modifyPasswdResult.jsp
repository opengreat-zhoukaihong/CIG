...
<%!
    ResultSet userInfo;
    String userID,passwd,oldPasswd1,oldPasswd2;
    String tempSqlStr;
    int errorCode;
%>
...
<%
        ...
        oldPasswd1 = request.getParameter("oldPasswd");
        passwd = request.getParameter("passwd1");

        tempSqlStr = "select passwd from users where userID = '" + userID + "'";
        userManager.setSqlStr(tempSqlStr);
        userInfo = userManager.getDatas();
        if(userInfo.next())
        {
          oldPasswd2 = userInfo.getString("passwd");
          userManager.disconn();

          if(!(oldPasswd1.equals(oldPasswd2)))
          {
%>
            <p>原密码输入有误！</p>
...
<%
          ...
          else
          {
            ...
            tempSqlStr = "update users set passwd='" + passwd +
               "',md_date=sysdate where userID = '" + userID + "'";
